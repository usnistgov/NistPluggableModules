...
tbr = t(1+br:end-br);                    % limited time vector with ignored values removed

%% this section gets a first guess at the step location
Y = (hilbert_w(Samples',window,padding))';              %find the analytic signal

% need a loop to individually calculate the gradients of each phase
Fi = zeros(nPhases,size(Y,2));dAi = Fi;
...
fm = median((Fi(:,1+br:end-br)),2);
detF = abs(Fi(:,1+br:end-br)-fm);    %detection signal limited by the ignored samples
[MF,IF] = max(detF,[],2);          % max value and index into the limited detection signal

% amplitude detection signal
am = median(dAi(:,1+br:end-br),2);
detA = abs(Fi(:,1+br:end-br)-am);    % amplitude detection signal limited by the ignored samples
